...
const config_1 = __importDefault(require("../src/shared/config"));
...
const main = async () => {
    ...
    if (roleCount > 0) {
        throw new Error('Roles are already created');
    }
    const roles = await prisma.role.createMany({
        data: [
            {
                name: role_constants_1.RoleName.Admin,
                description: 'Admin role',
            },
            {
                name: role_constants_1.RoleName.Client,
                description: 'Client role',
            },
            {
                name: role_constants_1.RoleName.Seller,
    ...
    const adminUser = await prisma.user.create({
        data: {
            ...
            phoneNumber: config_1.default.ADMIN_PHONENUMBER,
            roleId: adminRole.id,
        },
    });
    return {
